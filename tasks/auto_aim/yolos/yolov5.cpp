...
std::list<Armor> YOLOV5::detect(const cv::Mat & raw_img, int frame_count)// 被tasks/auto_aim/yolo.cpp调用，返回armors
{
  if (raw_img.empty()) {
...
std::list<Armor> YOLOV5::parse(
  double scale, cv::Mat & output, const cv::Mat & bgr_img, int frame_count)//被detect调用，返回armors
{
