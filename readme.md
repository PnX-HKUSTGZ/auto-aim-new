...
本项目经过多赛季迭代，逐步形成了当前的工程化能力与算法体系。在运行效率、模块解耦、调试闭环和稳定性方面持续改进，最终沉淀为当前版本的完整实现。

本项目进一步改进了识别器和决策器。其中，识别器由传统图像处理方案升级为基于神经网络的四点检测模型[1,2]，以期提高识别器的召回能力；决策器由经验主义的分段逻辑升级为基于轨迹优化的规划器，在简化代码的同时，实现更优的云台控制效果和开火决策逻辑，其思路会在后文详细介绍。
