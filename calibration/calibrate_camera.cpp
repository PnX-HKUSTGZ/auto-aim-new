  ...
  std::vector<std::vector<cv::Point2f>> & img_points)
{
  ...
  if (obj_points.empty()) {
    throw std::runtime_error(
      fmt::format(
        "No calibration images found in '{}'. Expected files like 1.jpg, 2.jpg, ...",
        input_folder));
  }
}
...
int main(int argc, char * argv[])
{
  ...
  auto resolved_input_folder = tools::resolve_path(input_folder).string();
  auto resolved_config_path = tools::resolve_path(config_path).string();
  ...
  std::vector<std::vector<cv::Point2f>> img_points;
  try {
    load(resolved_input_folder, resolved_config_path, img_size, obj_points, img_points);
